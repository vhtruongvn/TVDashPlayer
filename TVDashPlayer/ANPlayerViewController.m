...
- (void)showControlPanel:(BOOL)show {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        NSLog(@"Control panel animation finished");
    }];
}
...
- (void)controlPanelView:(ANControlPanelView *)theView sliderDidSlideToPosition:(CGFloat)pos {
    ...
    NSLog(@"ANPlayer : controlPanelView - slide timer to pos %f sec", pos);
...
- (void)waitForContent {
    if (!self.stopped){
        NSLog(@"ANPlayer : waitForContent");
        self.waitingForMediaContent = YES;
        NSLog(@"ANPlayer : waitForContent set to YES");
...
- (void)resumeFromContentWaiting {
    NSLog(@"ANPlayer : resumeFromContentWaiting");
...
- (void)startPlayback {
    ...
    NSLog(@"ANPlayer : startPlayback - _firstVideoSegmentCond signal");
...
- (void)scheduleVideoRefreshTimer {
    ...
    do {
        ...
        if (diff > 0){
            ...
            NSLog(@"ANPlayer : scheduleVideoRefreshTime - fireDate is in the past");
        }
    } while (diff < 0);
}
...
- (ANPicturesListElement *)nextPictureListElement {
    ...
    {
        NSLog(@"ANPlayer : nextPictureListElement - video ended");
        ...
        if (!self.isNextVideoReady) {
            NSLog(@"ANPlayer : nextPictureListElement - video - waitForContent");
            [self waitForContent];
            return element;
        }
        ...
        if (_nextVideo == nil) {
            NSLog(@"ANPlayer : nextPictureListElement - next video == NIL");
        }
...
- (void)switchToNextVideoSegment {
     NSLog(@"ANPlayer : switchToNextVideoSegment - next video segment number = %lu", (unsigned long)_nextVideo.videoData.segmentNumber);
...
- (void)calculateNextFramerate {
    ...
    if (diffAbs > ANSyncThreshold){
        ...
        NSLog(@"ANPlayer : calculateNextFramerate - framerate changed.\nNew framerate is %f",
             (double)_frameRate.num / _frameRate.den);
    }
    
    NSLog(@"ANPlayer : calculateNextFramerate - diff = %f", diff);
}
...
- (BOOL)prepareVideo {
    ...
    [_video startWork];
    NSLog(@"ANPlayer : prepareVideo - Created first video object %@", _video);
...
- (void)processNextVideoSegmentData:(ANVideoData *)videoData {
    ...
    NSLog(@"ANPlayer : processNextVideoSegmentData - process Next Video Segment");
    
    [_currentVideoFinishCond lock];
    while (!_allowNextVideoInitialization) {
        NSLog(@"ANPlayer : processNextVideoSegmentData - waiting for current video decoding END");
        [_currentVideoFinishCond wait];
        NSLog(@"ANPlayer : processNextVideoSegmentData - current video decoding ENDED");
    }
    _allowNextVideoInitialization = NO;
    ...
    self.nextVideoIsReady = YES;
    NSLog(@"ANPlayer : processNextVideoSegmentData - Created next video object %@ with segment number %lu", _nextVideo, (unsigned long)videoData.segmentNumber);
    ...
    NSLog(@"ANPlayer : processNextVideoSegmentData - isWaitingForMediaContent == %@", isWaiting ? @"YES" : @"NO");
    
    if (isWaiting){
        NSLog(@"ANPlayer : processNextVideoSegmentData - resume");
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)processNextAudioSegmentData:(ANAudioData *)audioData {
    ...
    NSLog(@"ANPlayer : processNextAudioSegmentData - process Next Audio Segment");

    [_currentAudioFinishCond lock];
    
    _nextAudio = [[ANAudioDecoder alloc] initWithAudioData:audioData];
    assert(_nextAudio);
    NSLog(@"ANPlayer : processNextAudioSegmentData - crated next audio object %@ with segment number %lu",
         _nextAudio, audioData.segmentNumber);
    
    _nextAudio.delegate = self;
    self.nextAudioIsReady = YES;
    
    NSLog(@"ANPlayer : processNextAudioSegmentData - Audio duration: %f", audioData.mediaDurationScaled);
    while (!_allowNextAudioInitialization) {
        NSLog(@"ANPlayer : processNextAudioSegmentData - waiting for current audio decoding END");
        [_currentAudioFinishCond wait];
        NSLog(@"ANPlayer : processNextAudioSegmentData - current audio decoding ENDED");
    }
    _allowNextAudioInitialization = NO;
    ...
    NSLog(@"ANPlayer : processNextAudioSegmentData - _nextAudioReadyCond signal");
...
{
    NSLog(@"ANPlayer : dashMultimediaManger:didDownloadFirstVideoSegment - process first media segments");
        ...
        didDownloadVideoData:(ANVideoData *)videoData
{
    dispatch_async(_videoSegmentQueue, ^{
        ...
        while (!_firstVideoSegmentIsDownloaded) {
            NSLog(@"ANPlayer : dashMultimediaManger:didDownloadVideoData - next VIDEO waiting for firstVideoSegmentDownloaded");
        ...
        didDownloadAudioData:(ANAudioData *)audioData
{
    dispatch_async(_audioSegmentQueue, ^{
        ...
        while (!_firstAudioSegmentIsDownloaded) {
            NSLog(@"ANPlayer : dashMultimediaManger:didDownloadAudioData - next AUDIO waiting for firstAudioSegmentDownloaded");
...
- (void)decoderDidFinishDecoding:(ANVideoDecoder *)video {
     NSLog(@"ANPlayer : decoderDidFinishDecoding - video did finish");
    if (!self.pausedForTimeShift){
        ANVideoData *videoData = [video videoData];
        NSInteger expectedFramesNumber = [videoData expectedFramesNumber];
        NSLog(@"ANPlayer : decoderDidFinishDecoding - frames in video = %lu", video.framesCount);
        ...
        NSLog(@"ANPlayer : decoderDidFinishDecoding - New framerate = %d / %d", _frameRate.num, _frameRate.den);
       
        _allowNextVideoInitialization = YES;
        NSLog(@"ANPlayer : decoderDidFinishDecoding - _currentVideoFinishCond signal");
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            NSLog(@"ANPlayer : decoderDidFinishDecoding - Send request for video segment downloading");
            if (_isStaticStream){
...
- (void)audioDidEnd:(ANAudioDecoder *)audio {
    NSLog(@"ANPlayer : audioDidEnd -_audioFinishCond signal");
    _allowNextAudioInitialization = YES;
    ...
    if ( (bytesRead > 0) && (bytesRead < ANAudioBufferSize)){
        if (!self.isNextAudioReady){
            if (_video.videoData.isLastSegmentNumber){
                ...
                NSLog(@"ANPlayer : readPCMAndPlay - Next audio isn't ready");
                ...
                dispatch_after(time, dispatch_get_main_queue(), ^{
                    NSLog(@"ANPlayer : readPCMAndPlay  -  audio - waitForContent");
                    [self waitForContent];
                });
            }
        } else {
            ...
            bytesRead += [_audio.ringBuffer read:audiodata + bytesRead
                                      withLength:ANAudioBufferSize - bytesRead
                                           block:1];
            NSLog(@"ANPlayer : readPCMAndPlay  - switch audio");
        }
        ...
        self.nextAudioIsReady = NO;
        if (_nextAudio == nil){
            NSLog(@"ANPlayer : readPCMAndPlay - NEXT AUDIO == NIL!!!!!!");
            [_audioLock unlock];
            return;
        }
        _audio = [self nextAudioDecoder];
        bytesRead = [_audio.ringBuffer read:audiodata
                                 withLength:ANAudioBufferSize
                                      block:1];
        NSLog(@"ANPlayer : readPCMAndPlay  - switch audio 2, Bytes read: %d", bytesRead);
    }
...
- (ANAudioDecoder *)nextAudioDecoder {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (_isStaticStream){
            NSLog(@"ANPlayer - Download next Audio segment");
...
- (void)setWaitingForMediaContent:(BOOL)waitingForMediaContent {
    ...
    if (waitingForMediaContent){
        NSLog(@"ANPlayer : setWaitingForMediaContent == YES");
    }
