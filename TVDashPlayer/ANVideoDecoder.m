...
static int ANReadFunction(void* opaque, uint8_t* buf, int buf_size) {
    ...
    if (vd->bytesLeft == 0) {
        NSLog(@"VIDEO DECODER %@ - End of video video data", vd);
        return -1;
    }
...
- (void)configVideoData {
    ...
    if (bytesLeft == 0){
        NSLog(@"VIDEO DECODER - Bytes left == 0");
    }
    bytesRead = 0;
}
...
- (void)demux_thread {
	...
	NSLog(@"VIDEO DECODER - video_demux_thread STARTED");
    AVPacket thePacket;
    int gotFrame = 0;
    _framesCount = 0;
    ...
    NSLog(@"VIDEO DECODER - videoSegment № %lu, video_demux_thread FINISHED", (unsigned long)self.videoData.segmentNumber);
