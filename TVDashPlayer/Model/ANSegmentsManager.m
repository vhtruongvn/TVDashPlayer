         ...
         withCompletionBlock:(ANCompletionBlock)completion
{
    ...
    __weak ANSegmentsManager *theWeakSelf = self;
     NSLog(@"SEGMENT_MANAGER - Start downloading video segment: %@", segmentUrl);
    [self.client downloadFile:segmentUrl
                  withSuccess:^(id response){
                      __strong ANSegmentsManager * theStrongSelf = theWeakSelf;
                      theStrongSelf.lastVideoSegmentData = response;
                      
                      NSLog(@"SEGMENT_MANAGER - Downloaded videoSegment: %@", segmentUrl);
                      if (completion){
                          completion(YES, nil);
                      }
                  }
         ...
         withCompletionBlock:(ANCompletionBlock)completion
{
    ...
    __weak ANSegmentsManager *theWeakSelf = self;
    NSLog(@"SEGMENT_MANAGER - Start downloading audio segment: %@", segmentUrl);
    [self.client downloadFile:segmentUrl
                  withSuccess:^(id response){
                       NSLog(@"SEGMENT_MANAGER - Downloaded audio segment: %@", segmentUrl);
                      __strong ANSegmentsManager * theStrongSelf = theWeakSelf;
